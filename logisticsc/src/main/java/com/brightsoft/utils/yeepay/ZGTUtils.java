...
public class ZGTUtils {
	...
	//各个API回调
	public static final String PAY_CAllBACK 					= "/yeePayCallBack/payCallBack.pay";
	...
	private static String callBackIP			= "";
	...
	static {
		...
		//从配置文件中获取ip
		callBackIP 	= merchantInfo.getValue("merchantCallbackIP");
	}
	...
	//生成回调url
	public static String buildCallBackUrl(HttpServletRequest request,String key) {
		String callUrl	= callBackIP +request.getContextPath() + key;
		return callUrl;
	}
	...
	//post请求
	public static Result httpPost(String url, String customernumber, String data,String[] responseHmacOrder) {
		...
		//掌柜通同步响应参数是一个json串，将该json串转成map。
		responseMap = JSON.parseObject(responseBody, new TypeReference<TreeMap<String, String>>() {});
		...
		//第六步 hmac签名验证
		if(!ZGTUtils.checkHmac(responseDataMap, responseHmacOrder)) {
	...
	//发起POST请求
	public static Result httpPost(String url, String data,String[] responseHmacOrder) {
		return httpPost(url, customernumber, data,responseHmacOrder);
	}
	...
	public static void setCallBackIP(String callBackIP) {
		ZGTUtils.callBackIP = callBackIP;
	}

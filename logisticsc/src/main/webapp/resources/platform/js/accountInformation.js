...
$(document).ready(function(){
    ...
    doCompany();
	$("#xiugaijiben").click(function() {
		return trueNameMsg() && addressjibenMsg() && updateUser();
	});	
	$("#trueName").focus(function() {
		$("#trueNameMsg").text("");
	});
	$("#addressjiben").focus(function() {
		$("#addressjibenMsg").text("");
	});
	$("#xiugaigongsi").click(function() {
		return companyName() && postCode() && companyAddress() && companyPhone() && legalPerson() && companyFax() && legalMobile() && companyTaxNo() && contacts() && financeEmail() && contactsMobile() && companyInfo() && qq() && businessLicenses() && companyPhotos() && legalPhotos() && cardphotos() && log() && verifUserCompany();
	});
...
//验证当前公司信息 是否存在审核
function verifUserCompany(){
	$.ajax({
		url:"/logisticsc/personalCenter/verifUserCompany.shtml",
		type:'post',
		dataType:'json',
		success:function(data){
			if(data.result==true){
				...
				$("#promptModal").modal('show');
	      		$("#promptModalMsgsDIV").html(data.msg);
	      		doCompany();
			}
		},
		error:function(data){
...
function updateCompany(){
	var option = {
        url : '/logisticsc/personalCenter/doUpdateCompanInfo.shtml',
        type : 'post',
        dataType : 'json',
...
/********************************************/
function trueNameMsg() {
	var trueName = $('#trueName').val();
	if(trueName == null ||trueName ==""){
		return true;
	}else if(/^[0-9a-zA-a]*$/.test(trueName)){
		$("#trueNameMsg").text("真实姓名只能是中文");
		return false;
	}else if(trueName.length < 2 || trueName.length > 6){
		$("#trueNameMsg").text("真实姓名字符长度为2-6位！");
		return false;
	}
	return true;
}
function addressjibenMsg() {
	var addressjiben = $('#addressjiben').val();
	if(addressjiben == null ||addressjiben ==""){
		return true;
	}else if(/^[0-9a-zA-a]*$/.test(addressjiben)){
		$("#addressjibenMsg").text("详细地址只能是中文！");
		return false;
	}else if(addressjiben.length < 3 || addressjiben.length > 30){
		$("#addressjibenMsg").text("详细地址字符长度为3-30位！");
		return false;
	}
	return true;
}
//修改用户信息
function updateUser(){
	var data = $("#update_userInfo").serialize();
	$.ajax({
		url:"/logisticsc/personalCenter/doUpdateUser.shtml",
		data:data,
		type:'post',
		dataType:'json',
		success:function(data){

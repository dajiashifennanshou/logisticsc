...
$(document).ready(function() {
	...
	$("#shenqing").click(function() {
		return companyName() && postCode() && companyAddress() && companyPhone() && legalPerson() && companyFax() && legalMobile() && companyTaxNo() && contacts() && financeEmail() && contactsMobile() && companyInfo() && qq() && businessLicenses() && companyPhotos() && legalPhotos() && cardphotos() && log() && agree() && verificationAddProvider();
	});
	...
	$("#zhifubaozhengjin").click(function(){
		return addUserBond();
	});
	$("#paymentMoney").focus(function() {
		$("#paymentMoneyMsg").text("");
	});
});
function agree(){
...
function addProvider(){
	  var option = {
        url : '/logisticsc/applyCenter/applyProvider.shtml',
        type : 'post',
        dataType : 'json',
        ...
        success : function(data,textStatus, XMLHttpRequest) {
        	if(data.result==true){
				...
				$("#successModalMsgs").html("申请物流商成功，审核通过，将会用短信通知你！");
			}else{
				$("#errorModal").modal('show');
    ...
    $("#addProvider").ajaxSubmit(option);
    return false;
}
//验证申请物流商
function verificationAddProvider(){
	$.ajax({
	       url :"/logisticsc/applyCenter/getUserBond.shtml",
	       type : 'POST',
	       dataType : 'json',
	       success:function(data){
	      	 if(data.result==true){
...
function displayBond(){
	$.ajax({
	       url :"/logisticsc/applyCenter/getBond.shtml",
	       type : 'POST',
	       dataType : 'json',
	       success:function(data){
	      	 if(data.result==true){
	      		$('body').css("overflow","hidden")
	      		$("#paymentMoney").val(data.data.money);
	      		$("#addBond").modal({show : true});
	      		$('body').css("overflow","scroll");
	   		 }else{
	   			$("#promptModal").modal('show');
				$("#promptModalMsgs").html("请联系客服，配置保证金!");
	   		 }
		 },error:function(data){
			 $("#errorModal").modal('show');
			 $("#errorModalMsgs").html("系统错误");
...
function shuaxin() {
	window.location.href="/logisticsc/applyCenter/toorderProvider.shtml";
}
//验证当前保证金
function verificationBond(){
	$.ajax({
	       url :"/logisticsc/applyCenter/getUserBond.shtml",
	       type : 'POST',
	       dataType : 'json',
	       success:function(data){
	      	 if(data.result==true){
	      		$("#promptModalMsgsDIV").html("");
	      		$("#promptModal").modal('show');
				$("#promptModalMsgs").html("已交纳保证金");
	   		 }else{
	   			//verificationCard();
	   			displayBond();
	   		 }
...
function verificationCard(){
	$.ajax({
	       url :"/logisticsc/applyCenter/getUserCard.shtml",
	       type : 'POST',
	       dataType : 'json',
	       success:function(data){
	      	 if(data.result==true){
	      		displayBond();
	   			...
	   			$("#promptModalMsgsDIV").html("");
	      		$("#promptModal").modal('show');
				$("#promptModalMsgs").html("<span>点击<a href='/logisticsc/personalCenter/toorderMyWallet.shtml'>请绑定一张银行卡</a></span>");
	   		 }
		 },error:function(data){
			 $("#errorModal").modal('show');
				$("#errorModalMsgs").html("系统错误");
...
/*function paymentMoneyMsg() {
	var zhuan_money = $("#paymentMoney").val();
	var zhuan_jine = $("#jine").text();
	zhuan_jine = parseFloat(zhuan_jine)
	if(null == zhuan_money || zhuan_money == ""){
		$("#paymentMoneyMsg").text("转账金额不能为空！");
		return false;
	}else if(!/^([0-9.]+)$/.test(zhuan_money)){
		$("#paymentMoneyMsg").text("转账金额只能输数字！");
		return false;
	}else if(zhuan_money >zhuan_jine){
		$("#paymentMoneyMsg").text("转账金额不足！");
		return false;
	}
	return true;
}*/
//支付保证金
//function addUserBond(){
//	var data = $("#add_bond").serialize();
//	$.ajax({
//	       url :"/logisticsc/applyCenter/doinsertUserBond.shtml",
//	       data:data,
//	       type : 'POST',
//	       dataType : 'json',
//	       success:function(data){

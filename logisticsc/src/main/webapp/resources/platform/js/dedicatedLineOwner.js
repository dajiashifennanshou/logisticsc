...
$(document).ready(function(){
	...
	$("#shenqingqiye").click(function() {
		companyName() && postCode() && companyAddress() && companyPhone() && legalPerson() && companyFax() && legalMobile() && companyTaxNo() && contacts() && financeEmail() && contactsMobile() && companyInfo() && qq() && businessLicenses() && companyPhotos() && legalPhotos() && cardphotos() && log() && agree() && applyDedicatedLine();
	});
...
//申请专线
function applyDedicatedLine(){
	var option = {
        url : '/logisticsc/applyCenter/applydedicatedLine.shtml',
        type : 'post',
        dataType : 'json',
        ...
        success : function(data) {
        	if(data.result==true){
        		...
        		$("#successModalMsgs").html("申请专线成功，审核通过，将会用短信通知你！");
			}else{
				$("#errorModal").modal('show');

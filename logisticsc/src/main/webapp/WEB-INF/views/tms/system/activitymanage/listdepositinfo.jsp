...
<body>
	<div class="panel">
		...
		<div class="panel-body">
			<form id="search_form" class="well form-inline">
				<input id="title" name="condition" type="text" placeholder="标题">
	...
	<script type="text/javascript">
		...
		BUI.use(['bui/grid','bui/data','bui/toolbar','bui/format','bui/select'],
			...
			store = new Store({
				url: '<%=request.getContextPath()%>/tms/sys/ratio/search.shtml',
				autoLoad:true,
				pageSize:10,
			});
			...
			function auditPass(){
				...
				if(length>0){
					...
					$.ajax({
						url:'<%=request.getContextPath()%>/tms/sys/ratio/audit/pass.shtml',
						type:'post',
						data:{ratioIds:array},
						dataType:'json',
			...
			function auditNotPass(){
				...
				if(length>0){
					...
					$.ajax({
						url:'<%=request.getContextPath()%>/tms/sys/ratio/audit/nopass.shtml',
						type:'post',
						data:{ratioIds:array},
						dataType:'json',
						success:function(data){

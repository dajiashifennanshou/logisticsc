...
<div class="header">
	...
	<div class="dl-log">
		<img class="toux" src="${configProps['resources']}/img/user-pic.jpg" /> <span
			class="dl-log-user">${sysUser.username}</span> <img class="dl-down"
...
<div class="name-info">
	...
	<div class="user-info">
		<span>${sysUser.username}</span> <br /> <i>${sysUser.username}</i>
	</div>
...
<script type="text/javascript">
	...
	function existSystem() {
		BUI.Message.Confirm('确认要退出吗？', function() {
			$.ajax({
				type : 'get',
				url : "${configProps['project']}/tms/sys/logout.shtml",
				cache : false,
				async : false,
				success : function() {
					location.href = "${configProps['project']}/tms/sys/tologin.shtml";
				}
			});
		}, 'question');
	}

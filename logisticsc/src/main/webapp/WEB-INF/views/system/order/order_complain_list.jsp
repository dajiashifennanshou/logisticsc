...
<div class="panel">
	...
	<script type="text/javascript">
		...
		BUI.use(['bui/grid','bui/data','bui/toolbar','bui/format','bui/form','bui/overlay'],
			...
			msg = BUI.Message,
			columns = [
				{title : '投诉时间',dataIndex :'complaintTimeStr', width:150,renderer:BUI.Grid.Format.datetimeRenderer},
	        	...
	        	{title : '投诉状态',dataIndex : 'status',renderer : function (value,obj) {
	        		if(obj.status == 0){
						return "已停用";
					}else{
						return "已启用"; 
				...
				{title : '操作',dataIndex : 'id',renderer : function (value,obj) {
					if(obj.status == 0 ){
						return "<a class='page-action' data-type='setTitle' href='${configProps['project']}/systemOrder/doGetComplainHandle.shtml?id="+obj.complainId+"'>查看</a> &nbsp;&nbsp;&nbsp;&nbsp;<a href='#' onclick='updateComplain("+obj.complainId+")'>启用</a>";
					}else{
						return "<a class='page-action' data-type='setTitle' href='${configProps['project']}/systemOrder/doGetComplainHandle.shtml?id="+obj.complainId+"'>查看</a> &nbsp;&nbsp;&nbsp;&nbsp;<a href='#' onclick='updateComplain("+obj.complainId+")'>禁用</a>"; 
					}
					}
				}
			];
			store = new Store({
				url: '<%=request.getContextPath()%>/systemOrder/getComplainList.shtml',
				autoLoad:true,
				proxy:{
					method:'post',
				},
		...
		function updateComplain(id){
			...
			$.ajax({
				cache: true,
				type: "POST",
				url:"${configProps['project']}/systemOrder/updateComplainHandle.shtml",
				data:{"id":complainId},
				async: false,

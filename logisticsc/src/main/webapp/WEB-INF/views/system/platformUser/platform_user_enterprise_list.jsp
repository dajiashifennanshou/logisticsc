		...
		BUI.use(['bui/grid','bui/data','bui/toolbar','bui/format','bui/form','bui/overlay'],
			...
			columns = [
					...
					{title : '操作',dataIndex : 'status',renderer : function (value,obj) {
						if(value==1){
							return "<a class='page-action' data-type='setTitle' href='${configProps['project']}/sysPlatformUser/getUserCompanyEnterprise.shtml?id="+obj.userId+"'>查看</a>&nbsp;&nbsp;&nbsp;&nbsp;<a style='cursor:pointer' class='page-action' data-type='setTitle' onclick='updateUserStatus("+obj.userId+")'>停用</a>  ";
						}else{
							return "<a class='page-action' data-type='setTitle' href='${configProps['project']}/sysPlatformUser/getUserCompanyEnterprise.shtml?id="+obj.userId+"'>查看</a>&nbsp;&nbsp;&nbsp;&nbsp;<a style='cursor:pointer' class='page-action' data-type='setTitle' onclick='updateUserStatus("+obj.userId+")'>启用</a>  ";
						}
					}}
			];
			store = new Store({
				url: '<%=request.getContextPath()%>/sysPlatformUser/getEnterprisePlatformUser.shtml',
				autoLoad:true,
				proxy:{
					method:'post',
				},

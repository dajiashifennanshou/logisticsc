	...
	<input type="text" id="temCompanyId" style="display: none;" readonly="readonly" value="${data.temCompanyId}" />
		...
		<div class="mt20">
				<div class="form-actions new-con-btn">
					<button type="submit" onclick="verifyUser(true)" class="button button-primary">通过</button>
					<button type="reset" onclick="verifyUser(false)" class="button">不通过</button>
				</div>
			</div>
		</div>
		<script type="text/javascript">
			function verifyUser(isPass){
				
				var userId = $("#userId").val();
				var temCompanyId = $("#temCompanyId").val();
				$.ajax({
		    		cache: true,
		    		type: "POST",
		    		url:"${configProps['project']}/sysPlatformUser/updateVerifyUser.shtml",
		    		data:{"userId":userId,"temCompanyId":temCompanyId,pass:isPass},
		    		async: false,
		    		...
		    		success: function(data) {
		    			...
		    			if(data.result){
			      			BUI.Message.Alert('操作成功！',function(){
			      				window.location="${configProps['project']}/system/pageJump.shtml?url=/system/platformUser/user_verify_list";

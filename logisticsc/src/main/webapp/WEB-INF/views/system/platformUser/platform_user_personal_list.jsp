		...
		BUI.use(['bui/grid','bui/data','bui/toolbar','bui/format','bui/form','bui/overlay'],
			...
			columns = [
					...
					{title : '操作',dataIndex : 'status',renderer : function (value,obj) {
						if(value==1){
							return "<a style='cursor:pointer' class='page-action' data-type='setTitle' onclick='updateUserStatus("+obj.userId+")'>停用</a>  ";
						}else{
							return "<a style='cursor:pointer' class='page-action' data-type='setTitle' onclick='updateUserStatus("+obj.userId+")'>启用</a>  ";
						}
					}}
			];
			store = new Store({
				url: '<%=request.getContextPath()%>/sysPlatformUser/getPersonalPlatformUser.shtml',
				autoLoad:true,
				proxy:{
					method:'post',
				},

<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html lang="en">
<head>
	<meta name="content-type" content="text/html; charset=UTF-8">
	<title>车辆信息</title>
	<link rel="stylesheet" href="/logisticsc/Clound/assets/css/ui.jqgrid.css" />
	<script type="text/javascript" src="/logisticsc/Clound/js/yc_public.js"></script>
	<script type="text/javascript" src="/logisticsc/Clound/js/JoinElseJs.js"></script>
	<script src="/logisticsc/Clound/assets/js/jqGrid/jquery.jqGrid.js"></script>
	<script src="/logisticsc/Clound/assets/js/jqGrid/i18n/grid.locale-cn.js"></script>
	<script type="text/javascript" src="/logisticsc/Clound/js/CarManage/car_manage.js"></script>
	<script type="text/javascript">
		$(function(){
			car_.initCars('#carType');
			car_.newGrid("#car_tab", "#car_pager");
			//下拉框初始化
			$('.select2').css("width","200px").select2({allowClear:true,minimumResultsForSearch: -1});
		});
	</script>
</head>
<body>
<div class="widget-box">
	<div class="widget-header widget-header-blue widget-header-flat">
		<h4 class="widget-title lighter">车辆信息</h4>
	</div>
	<div class="widget-header" style="padding-top: 6px;">
		<form id="car_form">
			<input class="col-xs-2" group="val" type="text" name="carNo" placeholder="请输入车牌号...">
			<select style="margin-left: 3px;" group="val" id="carType" name="carType" class="select2-container select2-allowclear select2 tag-input-style">
				<option value="">请选择车型</option>
			</select>
			<button style="margin-left: 3px;" type="reset" class="btn btn-purple btn-sm">
				<i class="ace-icon fa fa-undo bigger-110"></i>
				清空
			</button>
			<button type="button" class="btn btn-purple btn-sm" onclick="car_.condSearch();">
				<span class="ace-icon fa fa-search icon-on-right bigger-110"></span>
				确定
			</button>
		</form>
	</div>
	<table id="car_tab"></table>
	<div id="car_pager"></div>
</div>
</body>
</html>
